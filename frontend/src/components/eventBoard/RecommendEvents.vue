          ...
          <q-tab>
            ...
            >
              <q-card class="q-ma-sm" style="border-radius: 20px;">
                <q-card-section>
                  <div class="q-pa-sm d-flex" style="height: 180px; justify-content: space-between;">
                    <span class="text-weight-bold" style="font-size: 15px; margin-top: 20px;">{{ event.eventTitle }}</span>
                    <q-btn v-if="event.bookmark" @click="checkBookMark(index)" flat round color="primary" icon="bookmark" style="margin-left: 40px;"/>
                    <q-btn v-else @click="checkBookMark(index)" flat round color="primary" icon="bookmark_border" style="margin-left: 40px;"/>
                    <ul class="row" style="margin-top:0px;">
                      <li v-for="(tag, index) in event.eventTags" :key="index">
                        <div class="text-caption float-left" style="margin: 0px 5px 40px 0px;">#{{ tag }}</div>
                      </li>
                    </ul>
                      <div class="d-flex" style="clear: both; justify-content: space-between">
                        <span class="text-subtitle2 text-weight-bold text-primary">{{ event.eventDate }}</span>
                        <span>
                          <img
